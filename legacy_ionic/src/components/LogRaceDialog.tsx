...
export const LogRaceDialog = ({
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="max-w-3xl max-h-[90vh] max-h-[90dvh] flex flex-col bg-[#0a0a0a] border-2 border-racing-red/40 w-[95vw] sm:w-full p-0">
        <DialogHeader className="border-b-2 border-red-900/50 pb-3 px-4 sm:px-6 pt-4 sm:pt-6 flex-shrink-0">
          <DialogTitle className="text-lg sm:text-2xl font-black flex items-center gap-2 uppercase tracking-wider text-white drop-shadow-[0_2px_4px_rgba(0,0,0,0.8)]">
            <div className="w-1 h-5 sm:h-8 bg-racing-red rounded-full shadow-[0_0_8px_rgba(220,38,38,0.8)]" />
            {editMode ? 'Edit Race Log' : 'Log a Race'}
          </DialogTitle>
        </DialogHeader>

        <div className="space-y-[2vh] sm:space-y-[3vh] py-[2vh] sm:py-[3vh] px-[3vw] sm:px-[2vw] overflow-y-auto flex-1 overscroll-contain">
          {/* Race Info Section */}
        ...
        <div className="flex flex-col sm:flex-row justify-end gap-2 sm:gap-3 p-[2vh] sm:p-[3vh] border-t-2 border-red-900/50 flex-shrink-0 bg-[#0a0a0a] sticky bottom-0">
          <Button
            variant="outline"
            onClick={() => setOpen(false)}
            className="order-2 sm:order-1 border-2 border-red-900/40 bg-black/60 text-white hover:bg-red-900/20 font-bold uppercase tracking-wider min-h-[48px] px-6 w-full sm:w-auto"
          >
            Cancel
          </Button>
          <Button
            ...
            disabled={loading}
            className="order-1 sm:order-2 bg-racing-red hover:bg-red-600 text-white shadow-lg shadow-red-500/50 border-2 border-red-400 font-black uppercase tracking-wider min-h-[48px] px-6 w-full sm:w-auto"
          >
            {loading ? (
